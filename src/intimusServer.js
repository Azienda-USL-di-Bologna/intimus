...
*/
var Redis = require("redis"),
    redis = Redis.createClient({port:config.REDIS_PORT, host:config.REDIS_HOST, auth_pass:config.REDIS_AUTH_PASS,
        retry_strategy: function (options) {
            return 5000;
        
    }});
var redisq = Redis.createClient({port:config.REDIS_PORT, host:config.REDIS_HOST, auth_pass:config.REDIS_AUTH_PASS,
    retry_strategy: function (options) {
        return 5000;
    }
});
var redisr = Redis.createClient({port:config.REDIS_PORT, host:config.REDIS_HOST, auth_pass:config.REDIS_AUTH_PASS,
    retry_strategy: function (options) {
        return 5000;
    }
});

redis.on("connect", function (event) {redis.select(config.REDIS_DB,function (event) { redis.del(SPARKS_SET, CLIENT_HASH_NAME, CLIENT_PRESENCE_SUBSCRIBERS) })});
redisq.on("connect", function (event) {
    redisq.select(config.REDIS_DB, function (event) {
...
redisr.on("connect", function (event) {
    redisr.select(config.REDIS_DB, function (event) {
